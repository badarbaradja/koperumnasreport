...
13. **`GET /api/pos-handoff` (`lib/posHandoff.ts`) — PENGECUALIAN TERTULIS (25 September 2026, disetujui pemilik proyek):** satu-satunya tempat di repo ini yang menandatangani token untuk sistem LAIN (pos-fnb, proyek Supabase terpisah total). Bukan pengecualian RLS repo ini sendiri (route ini cuma baca sesi cookie milik pemanggil sendiri, sama seperti route lain) — tapi tetap dicatat di sini karena polanya gampang disalahgunakan kalau tidak dipagari eksplisit. Batasnya, jangan dilebarkan tanpa persetujuan baru:
    1. Token HANYA membawa `email` + `uid` milik user yang SEDANG login sungguhan di request ini (`supabase.auth.getUser()`) — TIDAK PERNAH atas nama orang lain, dan TIDAK PERNAH membawa role/peran/izin apa pun dari sistem ini.
    2. Kedaluwarsa 60 detik, ditandatangani `POS_HANDOFF_SECRET` (rahasia server, sama nilainya dengan `POS_HANDOFF_SECRET` di pos-fnb — repo terpisah, lihat `src/lib/auth/handoff-token.ts` di sana untuk sisi verifikasi).
    3. Siapa boleh MELIHAT tombolnya diatur `bolehLihatTautanPos()` (`lib/posLink.ts`, daftar email eksplisit) — TAPI siapa yang BENAR-BENAR bisa masuk ke pos-fnb ditentukan SEPENUHNYA oleh pos-fnb sendiri (`report_identity_links`, dikelola owner pos-fnb lewat `/team` di sana). Menambah email ke daftar di sini TANPA pemetaan yang cocok di pos-fnb cuma menampilkan tombol yang berakhir "akun toko belum disiapkan" — tidak berbahaya, tapi sia-sia, jangan lupa dua-duanya jalan bersamaan.
    4. Jangan pernah menambah field baru ke payload token (role, outletId, dst.) — itu akan membuat repo ini diam-diam menjadi sumber otoritas untuk sistem lain, persis yang dihindari desain ini.
