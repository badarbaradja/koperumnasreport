 ...
 * form itu dulu ("Laporan Harian Ita") sebenarnya dua pekerjaan berbeda
 * dijadikan satu: unit usaha thrifting milik Ita sendiri, dan kontrol
 * F&B per outlet yang sekarang bisa dikerjakan lebih dari satu orang
 * (Ita + Rika). Dipecah supaya bentuk data (satu form = satu tanggung
 * jawab) tidak lagi memaksa keduanya jadi satu laporan.
 *
 * Ini bagian THRIFTING (bagian 1-4 dokumen asli) -- scope 'global', SATU
 * laporan per hari, TIDAK berubah dari `f16-ita.ts` sebelumnya (field key,
 * label, validasi persis sama). Bagian kontrol F&B per outlet (bagian
 * 5-9) ada di `forms/f17-kontrol-fnb.ts`, `scope:'outlet'`.
 *
 * Blok "Stock Opname Mingguan" (khusus Senin, `hanyaHari: [1]` -- mekanisme
 * YANG SUDAH ADA, forms/validasi.ts `blokBerlakuHariIni`, TIDAK dibuat
 * baru) dipecah juga -- bagian THRIFTING-nya di sini, bagian per-outlet di
 * `f17-kontrol-fnb.ts`. Enam field penutup (daftar barang selisih dkk)
 * ada di KEDUA form dengan KUNCI SAMA tapi LABEL beda-beda menyebut
 * miliknya siapa (instruksi eksplisit user, prinsip sama dengan "Dapur
 * bersih (sebelum buka)" Manager Resto) -- supaya pengisi tidak bingung
 * melihat label yang sama dua kali di dua form berbeda.
 */
export const f16Thrifting: FormSchema = {
  key: 'thrifting',
  nama: 'Laporan Harian Thrifting (Ita)',
  navLabel: 'Lapor Thrifting',
  scope: 'global',
  blocks: [
    {
      id: 'thrifting',
    ...
    {
      id: 'stock_opname_thrifting',
      judul: 'Stock Opname Mingguan -- Thrifting (khusus Senin)',
      hanyaHari: [1],
      ...
      fields: [
        ...
        { key: 'so_daftar_barang_selisih', label: 'Daftar barang selisih (thrifting)', type: 'teks_panjang' },
        { key: 'so_penyebab_selisih', label: 'Penyebab selisih (thrifting)', type: 'teks_panjang' },
        { key: 'so_tindakan_perbaikan', label: 'Tindakan/perbaikan (thrifting)', type: 'teks_panjang' },
        { key: 'so_kebutuhan_minggu_depan', label: 'Kebutuhan stok minggu depan (thrifting)', type: 'teks_panjang' },
        { key: 'so_rab_kebutuhan', label: 'RAB kebutuhan thrifting minggu depan', type: 'uang' },
        { key: 'so_diajukan_persetujuan', label: 'Diajukan ke persetujuan (thrifting)', type: 'ya_tidak' },
      ],
    },
    blokKeputusanCeo('Rekap Thrifting Hari Ini', 'Status hari ini'),
  ],
};
