...
import { pesanErrorMasuk } from '../lib/pesanError';
...
async function muatProfilPeran(userId: string) {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      if (data.session) {
        const hasil = await muatProfilPeran(data.session.user.id);
        if (!aktif) return;
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(async (_event, sesiBaru) => {
      setSession(sesiBaru);
      if (sesiBaru) {
        const hasil = await muatProfilPeran(sesiBaru.user.id);
        setProfile(hasil.profile);
        setRoles(hasil.roles);
        setAssignments(hasil.assignments);
      } else {
