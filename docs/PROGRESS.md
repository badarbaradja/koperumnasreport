...
| 04 | Row Level Security | ✅ (kode + skema; matriks uji 14-baris masih CHECKPOINT 2) | **Dijalankan lewat `npm run db -- supabase/migrations/0002_rls.sql`** (exit code 0, tanpa error), lalu diverifikasi langsung: `rls_aktif = true` di seluruh 11 tabel (`pg_class.relrowsecurity`); 24 policy ada persis sesuai spesifikasi termasuk konfirmasi **`report_nudge` memang tidak ada** (`pg_policies`); 4 fungsi/RPC ada — `has_role`, `can_see_report`, `tagih_laporan`, `set_updated_at` (`information_schema.routines`); 2 policy storage `bukti_upload`/`bukti_read` ada (`pg_policies` schema `storage`). Semua dicek satu-satu lewat query nyata, bukan diasumsikan dari "migrasi sukses". | Matriks uji 14-baris `04-CATATAN-TEKNIS.md` §4 (skrip Checkpoint 2 dengan penyamaran `set role authenticated`) **belum dijalankan** — menunggu instruksi user secara eksplisit sebelum Run, sesuai permintaannya. |
