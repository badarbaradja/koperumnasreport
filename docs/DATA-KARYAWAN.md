...
| Ita | Thrifting & Kontrol F&B | `thrifting` (global) + `kontrol_fnb` @ Indokopi Jatinegara -- **RENCANA, belum dijalankan** | `karyawan` |
| Pak Tri | Kendaraan | `kendaraan` (menjawab §2 nomor 6 -- form ini sebelumnya tanpa pengisi) | `karyawan` |
| Mba Rika | Kontrol F&B (bersama Ita) | `kontrol_fnb` @ Indosteak Cempaka + @ Indosteak Pekansari, `wajib_pte=false` ("Fokus kontrol stok, tidak menangani marketing") -- **RENCANA, belum dijalankan** | `karyawan` |
...
⚠️ **Assignment Ita & Rika di atas MASIH RENCANA -- SENGAJA belum dijalankan** (instruksi eksplisit user, 30 Agustus 2026): CEO belum mengonfirmasi siapa mengontrol outlet mana secara resmi, dan itu bisa diatur sendiri kapan saja lewat tab Admin "Penugasan" begitu 39 akun asli dibuat -- tidak perlu menunggu agent lagi untuk perubahan semacam ini ke depannya.
