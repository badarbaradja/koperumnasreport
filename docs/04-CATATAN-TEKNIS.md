...
14. **Kegagalan yang tidak bersuara -- pola `{ data, error }` yang tidak diperiksa.** Ditemukan ULANG TIGA KALI (bukan kebetulan): (1) 31 Agustus 2026, tombol tambah outlet -- `TabOutlet` tidak pernah merender `tambah.isError`, jadi galat `null value in column "slug"` terlihat seperti "tidak terjadi apa-apa"; (2) tab Admin lain (Lokasi, Penugasan, Policy, Pengguna, Titik Absen) sama sekali tidak menampilkan galat mutasi, ditemukan sekaligus saat memperbaiki (1); (3) 7 September 2026, `scripts/tmp-uji-kebersihan-bersama.mjs` -- blok `finally` memanggil `admin.auth.admin.updateUserById(id, { password: 'admin123' })` untuk memulihkan DUA akun karyawan SUNGGUHAN (Qasim, Ryan) yang dipakai login sungguhan saat uji, TANPA memeriksa `{ error }` yang dikembalikan. Panggilan itu gagal senyap untuk keduanya, skrip tetap mencetak "✅ SEMUA LOLOS" dan "Dibersihkan", dan kedua karyawan nyaris tidak bisa masuk kerja esoknya -- password asli mereka sudah tergantikan string acak yang tidak pernah dicatat di mana pun (lihat docs/PROGRESS.md, insiden Qasim & Ryan).

    Akar yang sama di ketiganya: klien Supabase (`supabase-js`, `@supabase/ssr`) TIDAK PERNAH `throw` sendiri -- setiap panggilan tulis (`insert`/`update`/`upsert`/`delete`/`rpc`/`auth.admin.*`) mengembalikan `{ data, error }`, dan kalau `error` tidak diperiksa, kegagalan itu tidak berbunyi sama sekali: tidak ada exception, tidak ada baris merah di log, kode setelahnya tetap berjalan seolah berhasil.

    **Aturan mutlak sejak insiden ini:**
    - **Setiap pemanggilan yang mengembalikan `{ data, error }` WAJIB memeriksa `error`.** Di `lib/api/*.ts` (mutation hook TanStack Query): `if (error) throw error;` -- biarkan `useMutation` menaruhnya di `mutation.error`, tampilkan lewat `pesanKesalahanDb()` di komponen. Di `app/api/*/route.ts`: kembalikan `NextResponse.json({ error: ... }, { status: ... })`, JANGAN diam-diam lanjut ke langkah berikutnya seolah berhasil kecuali ada alasan eksplisit & ditulis kenapa langkah itu boleh gagal tanpa membatalkan keseluruhan (lihat `reset-password/route.ts` -- kegagalan `reset_password_log` sengaja tidak membatalkan request karena password-nya SENDIRI sudah terlanjur berubah, dikomentari eksplisit kenapa). Di skrip sekali-pakai (`scripts/*.mjs`): sama, `if (error) throw`/`console.error` lalu hentikan, JANGAN lanjut mencetak "berhasil".
    - **Skrip yang mencetak "berhasil"/"dibersihkan" WAJIB membuktikannya dengan MEMBACA ULANG dari database setelahnya, bukan dari nilai kembalian saja.** Nilai kembalian cuma membuktikan permintaan TERKIRIM, bukan bahwa perubahannya BENAR-BENAR tersimpan (contoh pola pembuktian: bandingkan `auth.users.updated_at` dengan `last_sign_in_at` -- kalau sama persis, tidak ada apa pun yang menyentuh baris itu sejak login terakhir, walau kode "berhasil" menyetel password baru).
    - **Skrip uji TIDAK PERNAH menyentuh akun orang sungguhan** -- lihat aturan terpisah, ditemukan bersamaan lewat insiden yang sama (Qasim & Ryan). Dua pelajaran ini saling memperkuat: kalaupun error DIPERIKSA dengan benar, memulihkan password akun sungguhan ke "admin123" cuma benar KALAU itu memang password sebelumnya -- skrip tidak pernah bisa tahu itu. Satu-satunya cara aman dari kedua sisi: pakai akun uji khusus (`scripts/buat-akun-uji.mjs`), bukan akun siapa pun yang sungguhan bekerja.

15. **`jam_tutup <= jam_buka` berarti tutup di HARI BERIKUTNYA -- BUKAN salah ketik.** Ditetapkan migrasi `0055_jadwal_operasional_outlet.sql` (10 September 2026, jadwal operasional per outlet per hari): kafe Indokopi buka `09:00`, tutup `03:00` -- angka jam TUTUP secara numerik "lebih kecil" dari jam BUKA. Orang yang membaca baris `jadwal_operasional` (`jam_buka='09:00', jam_tutup='03:00'`) tanpa tahu aturan ini akan mengira datanya terbalik/salah input.

    **Sengaja TIDAK ada kolom penanda `tutup_besok` terpisah** -- disetujui eksplisit CEO: dua kolom (jam + penanda boolean) berisiko tidak sinkron kalau salah satu diubah tanpa yang lain (mis. jam diedit lewat satu jalur, penanda lewat jalur lain). Aturan tunggal `jam_tutup <= jam_buka → tutup besok` cukup dan tidak ambigu KARENA seluruh domain bisnis ini (Indosteak, Indokopi) selalu buka pagi -- tidak ada outlet yang buka lewat tengah malam, jadi tidak ada kasus di mana "jam tutup lebih kecil" bisa berarti sesuatu selain "menyeberang tengah malam".

    **PENTING -- aturan ini BELUM dipakai untuk menghitung apa pun di kode manapun** per migrasi 0055. Deadline Laporan Kebersihan (satu-satunya pemakai `jadwal_operasional` sejauh ini) cuma pernah butuh `jam_buka` (pagi, sebelum outlet buka) -- tidak pernah `jam_tutup`. Kolom `jam_tutup` disimpan BENAR sejak awal supaya siap kalau ada fitur nanti yang butuh menghitung dari jam tutup (mis. checklist penutupan) -- tapi jangan asumsikan ada logika "hari mana" yang sudah berjalan hanya karena kolomnya ada.

16. **Input terkontrol (`value={dataServer}`) yang mengirim mutasi async PER FIELD, bukan sekaligus, bisa saling menimpa dengan nilai basi.** Ditemukan 10 September 2026 saat mengisi jadwal operasional lewat Admin: dua `<input type="time">` (jam buka, jam tutup) di baris yang sama, masing-masing `onChange`-nya langsung memanggil mutasi TanStack Query membawa nilai field SATUNYA dari closure data server (`h.jamTutup` saat jam buka diubah, dan sebaliknya). Mengisi jam buka lalu jam tutup dengan cepat: mutasi pertama tersimpan benar, tapi kalau query belum sempat di-invalidate+refetch sebelum field kedua diubah, mutasi KEDUA mengirim nilai field pertama yang MASIH LAMA (closure belum ter-refresh) -- field yang baru saja diisi bisa balik jadi kosong, TANPA galat apa pun (kedua mutasi individual "berhasil").

    **Perbaikannya**: baris dengan lebih dari satu field yang disimpan BERSAMA (satu baris = satu unit data) pakai draft LOKAL (state komponen, bukan data server) untuk SEMUA field dalam baris itu, dengan SATU tombol simpan yang mengirim seluruh draft sekaligus -- lihat `JadwalOutlet` di `app/admin/page.tsx`. Pola yang SUDAH BENAR sejak awal dan bisa dicontoh: `TabPolicy` (draft `textarea` + tombol Simpan per baris, migrasi lama). Kalau tiap field independen boleh tersimpan sendiri-sendiri tanpa pernah saling bergantung pada field lain di baris yang sama (kolom `aktif` outlet, misalnya), auto-save per field tetap aman -- jebakan ini KHUSUS field-field yang harus konsisten satu sama lain dalam satu baris.
